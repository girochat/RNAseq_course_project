...
do
	...
	file_id=$(basename $file | awk -F "_ab" '{print $1}')
...
target_id	length	eff_length	est_counts	tpm' temp.tsv.sorted > temp.tsv	
	...
	# add the corresponding gene name and its known status to the detected transcript
	LANG=en_EN join --header -1 1 -2 1 $HOME/analysis/transcripts_genes_id.tsv.sorted temp.tsv | awk 'BEGIN{OFS="\t"}{print $1,$2,$3,$4,$5,$6,$7}' > ${file_id}_final_abundance.tsv
